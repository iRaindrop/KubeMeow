---
title: "Flatcar Orchestration & Container Runtimes"
type: concept
sources:
  - raw/topics/flatcar/content/docs/latest/orchestrate/
related:
  - "[[flatcar-overview]]"
...
**Custom Docker/containerd version:** Disable the built-in sysexts (see [[flatcar-provisioning]]) and place custom binaries from `sysext-bakery` or `/opt/bin/`.

**Registry authentication:** Credentials in `~/.docker/config.json` or `/etc/docker/config.json` for system-wide access.

**Switching to unified cgroups (cgroupv2):** Covered in `orchestrate/containers/switching-to-unified-cgroups.md` — relevant for Kubernetes nodes.

**Incus (LXC/VM manager):** Opt-in sysext (`incus`) since Flatcar 4285.0.0. See `orchestrate/containers/incus.md`.

## Kubernetes (`orchestrate/kubernetes/`)
...
**Reboot coordination:** Use [FLUO](https://github.com/flatcar/flatcar-linux-update-operator/) (preferred) or [Kured](https://kured.dev/) instead of locksmithd for Kubernetes nodes.
