---
title: "Flatcar OS Configuration"
type: concept
sources:
  - raw/topics/flatcar/content/docs/latest/os-config/
related:
  - "[[flatcar-overview]]"
...
## Host Configuration (`os-config/host-config/`)
...
**Setting update strategy via Butane** (see also [[flatcar-updates]]):
```yaml
variant: flatcar
version: 1.0.0
storage:
...
## Networking (`os-config/network/`)

Flatcar uses `systemd-networkd` for all network configuration.

**Custom network config via Butane:**
```yaml
variant: flatcar
version: 1.0.0
storage:
